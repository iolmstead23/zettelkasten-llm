import { Fragment, useCallback, useRef } from "react";
import { Dialog, Transition } from "@headlessui/react";
import { ExclamationTriangleIcon } from "@heroicons/react/24/outline";
import { useNotifyToggleContext, useSaveToggleContext } from "components/providers/subproviders/ToggleProvider";
import { useLogger } from "components/logging/LogWrapper";
import { LogEntryMetadata, LogLevel } from "types/types";
import { useNotifyContentContext } from "@/components/providers/subproviders/NotificationProvider";

/**
 * Save confirmation dialog component
 * @component
 * @example
 * return (
 *   <SaveDialogueItem
 *     onSave={() => handleSave()}
 *     onDontSave={() => handleDontSave()}
 *   />
 * )
 *
 * @remarks
 * Displays a modal dialog for confirming file save actions
 * Features:
 * - Save confirmation message
 * - Save/Don't Save options
 * - Callback handlers for actions
 *
 * @param {object} props
 * @param {() => void} props.onSave - Callback for save action
 * @param {() => void} props.onDontSave - Callback for don't save action
 * @returns {JSX.Element} Save confirmation dialog
 */
const SaveDialogueItem = ({
  onSave,
  onDontSave,
}: {
  onSave: () => void;
  onDontSave: () => void;
}): React.JSX.Element => {
  const cancelButtonRef = useRef(null);
  const { saveIsOpen, setSaveIsOpen } = useSaveToggleContext();
  const { addLogs } = useLogger();
  const { notifyContent, setNotifyContent } = useNotifyContentContext();
  const { notifyToggle, setNotifyToggle } = useNotifyToggleContext();

  const handleLogger = useCallback(
    async ({
      message,
      level,
      metadata,
    }: {
      message: string;
      level: LogLevel;
      metadata?: LogEntryMetadata;
    }) => {
      try {
        await addLogs({
          message,
          level,
          metadata: { ...metadata, component: "SaveDialogueItem" },
        });
      } catch (error: any) {
        console.error("Log submission error", error.message);
      }
    },
    [addLogs]
  );

  return (
    <Transition.Root show={saveIsOpen} as={Fragment} appear={true}>
      <Dialog
        as="div"
        className="relative z-40"
        initialFocus={cancelButtonRef}
        onClose={() => setSaveIsOpen(false)}
      >
        <Transition.Child
          as={Fragment}
          enter="ease-out duration-300"
          enterFrom="opacity-0"
          enterTo="opacity-100"
          leave="ease-in duration-200"
          leaveFrom="opacity-100"
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" />
        </Transition.Child>
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
              enterTo="opacity-100 translate-y-0 sm:scale-100"
              leave="ease-in duration-200"
              leaveFrom="opacity-100 translate-y-0 sm:scale-100"
              leaveTo="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                <div className="sm:flex sm:items-start">
                  <div className="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10">
                    <ExclamationTriangleIcon
                      className="h-6 w-6 text-red-600"
                      aria-hidden="true"
                    />
                  </div>
                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    <Dialog.Title className="text-lg font-semibold">
                      Save Confirmation
                    </Dialog.Title>
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        File is unsaved. Do you wish to save now?
                      </p>
                    </div>
                    <div className="mt-2">
                      <div>
                        <label htmlFor="save" className="sr-only">
                          Save
                        </label>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                  <button
                    type="button"
                    className="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto"
                    onClick={() => {
                      onSave();
                      handleLogger({
                        message: "Save confirmed",
                        level: "DEBUG",
                      });
                      setNotifyContent({ type: "success", message: "Save success!" });
                      setNotifyToggle(true);
                    }}
                  >
                    Save
                  </button>
                  <button
                    type="button"
                    className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto"
                    onClick={() => {
                      onDontSave();
                      handleLogger({
                        message: "Save cancelled",
                        level: "DEBUG",
                      });
                    }}
                    ref={cancelButtonRef}
                  >
                    Dont Save
                  </button>
                </div>
              </Dialog.Panel>
            </Transition.Child>
          </div>
        </div>
      </Dialog>
    </Transition.Root>
  );
};

export default SaveDialogueItem;
